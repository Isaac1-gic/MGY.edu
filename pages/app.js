...
// 1. Use the most recent stable model ID
const model = getGenerativeModel(ai, { 
  model: "gemini-3-flash-preview", // Update from preview to stable
  location: "global" 
}, {
  tools: [
    { googleSearch: {} } 
...
async function run() {
  // 1. Force the AI to acknowledge TODAY is March 21, 2026
  const prompt = "Search Google for news today, March 21, 2026. Give me the current status of the war in the Middle East (Iran/Israel) and Ukraine. Do not use your old 2024 data.";

  // 2. Use the 'dynamic' threshold (This tells the AI: "If you aren't 100% sure, SEARCH")
  const model = getGenerativeModel(ai, { 
  ...
  toolConfig: {
    googleSearchRetrieval: {
      dynamicThreshold: 0.01 // Forces search for almost every query
    }
  }
});
  ...
  // LOGIC CHECK: If this is still undefined, the Search Tool is disabled in your Firebase Console
  console.log("Sources used:", result.response.candidates[0].groundingMetadata);
...
<script type="module">
  ...
  // Assuming you have your Firebase 'db' and 'ref' already initialized
  // import { getDatabase, ref, get, child } from "https://www.gstatic.com/firebasejs/10.x/firebase-database.js";

  const genAI = new GoogleGenerativeAI("AIzaSyD1r8rclrIPVQg1Nndd54axiQaJgyvPLTk");

  // Initialize the model with the 2026 Search and Database Tools
  const model = genAI.getGenerativeModel({
    model: "gemini-2.5-flash", // The stable 2026 free-tier model
    tools: [
      { googleSearch: {} } 
     /** `{
        functionDeclarations: [{
          name: "getTrackerData",
          description: "Fetch MSCE study progress or results for a specific subject from the user's database.",
          parameters: {
            type: "OBJECT",
            properties: {
              subject: { type: "string", description: "The MSCE subject (e.g., Biology, Math)" }
            }
          }
        }]
      }`**//**
  ...
  // 1. The Bridge Function: This actually reads from YOUR Firebase
  async function getTrackerData(subject) {
    console.log("AI is asking for:", subject);
    const dbRef = ref(getDatabase());
    const snapshot = await get(child(dbRef, `users/YOUR_USER_ID/results/${subject}`));
    if (snapshot.exists()) {
      return snapshot.val(); // Returns { grade: "A", status: "completed", etc. }
    } else {
      return "No data found for this subject.";
    }
  }

  // 2. The Interaction Loop: Handles the "Back-and-Forth"
  async function askAI(userInput) {
    const chat = model.startChat();
    let result = await chat.sendMessage(userInput);
    let response = result.response;

    // Check if the AI wants to call your database function
    const calls = response.functionCalls();
    if (calls && calls.length > 0) {
      const call = calls[0];
      if (call.name === "getTrackerData") {
        // Execute the local Firebase read
        const data = await getTrackerData(call.args.subject);
        
        // Send the database info back to the AI to finish the answer
    ...
    // Display final result in your UI
    //document.getElementById("output").innerText = 
		alert(response.text());
    console.log("Sources used:", response.groundingMetadata);
  }
	
async function AI(prompt) {
    const displayElement = document.getElementById("ai-response");
    displayElement.innerText = ""; // Clear old text before starting

    try {
      // 1. Start the Stream (Input: String -> Output: Chunks)
      const result = await model.generateContentStream(prompt);

      // 2. The "Typing" Loop
      // 'for await' stays open until the AI is finished talking
      for await (const chunk of result.stream) {
        const chunkText = chunk.text(); // Get the new piece of text
        
        // 3. Append to UI immediately (The "Typing" effect)
        displayElement.innerText += chunkText; 
        
        // Optional: Auto-scroll to the bottom as it types
        window.scrollTo(0, document.body.scrollHeight);
      }

      console.log("AI has finished typing.");
